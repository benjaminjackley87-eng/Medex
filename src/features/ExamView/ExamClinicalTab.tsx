...
interface ExamClinicalTabProps {
  exam: Examination;
  isEditMode?: boolean;
  updateGeneralField: (field: keyof Examination, value: Examination[keyof Examination]) => void;
}

export const ExamClinicalTab: React.FC<ExamClinicalTabProps> = ({
  exam,
  isEditMode,
  updateGeneralField
}) => {
    ...
    <div className="animate-in slide-in-from-bottom-6 duration-700 space-y-16">
      ...
      <section className="space-y-8">
        ...
        <div className="grid grid-cols-1 gap-6">
          {(exam.diagnosticCriteria || []).map((dc, i) => (
            <div
              key={i}
              className="group relative p-8 bg-slate-950/40 rounded-3xl border border-white/5 shadow-sm hover:shadow-md transition-all"
            >
              <h4 className="text-sm font-black text-white mb-4 uppercase tracking-tight border-b border-white/5 pb-4">
                {dc.condition}
              </h4>
              <ul className="space-y-3">
                {dc.criteria.map((criterion, j) => (
                  <li
                    key={j}
                    className="flex gap-3 text-[13px] text-slate-400 font-medium leading-relaxed"
                  >
          ...
          {(!exam.diagnosticCriteria || (exam.diagnosticCriteria || []).length === 0) && (
            <p className="col-span-full text-center py-12 text-slate-400 text-xs font-bold uppercase tracking-widest">
              No diagnostic criteria documented.
            </p>
          )}
        </div>
      </section>
